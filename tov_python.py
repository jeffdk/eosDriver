...
class tovinfoclass(object):
    ...
    def __init__(self,nrhos):
        ...
        self.rmax = 0.0
        self.nrhos = nrhos
        self.eostable = np.zeros((self.nrhos,2))
        self.logrhos = np.zeros((self.nrhos))
        self.eoslrhomin = 0.0
        self.eoslrhomax = 0.0
...
#        rhs[2] = 4.0*pi*r**2 * rho
        ...
        if(press > tovinfo.minpress and not tovinfo.stopflag):
            rhs[0] = - ggrav / r**2 * \
                ...
                (mass + 4.0*np.pi*r**3 * press/clite**2) * \
                1.0/(1.0 - 2.0*ggrav*mass/r/clite**2)
        else:
            rhs[0] = 0.0

        # gravitational mass 
        rhs[1] = 4.0*np.pi*r**2 * u

        # baryonic mass
        rhs[2] = 4.0*np.pi*r**2 * rho * \
            1.0/np.sqrt(1.0-2.0*ggrav*mass/r/clite**2)
...
def tov_integrate(rho_c,tovinfo):
    ...
    tovdata = np.zeros((nzones,9))
    rhos = np.zeros(nzones)
    # 0 -- press
    # 1 -- mgrav
    # 2 -- mbary 


    tovout = np.zeros((nzones,10))
    # 0 -- rho
    # 1 -- press
    # 2 -- eps
    # 3 -- mgrav
...
def tov_sequence(rho1,rho2,nsteps,tovinfo):
    ...
    # set up rhos
    rhos = np.zeros(nrho)
    dlrho = (np.log10(rho2)-np.log10(rho1)) / (nrho-1)
    for i in range(len(rhos)):
        rhos[i] = np.log10(rho1) + i*dlrho
...
eosfilename = "Hempel_SFHo_eostable_BetaEq_T=00.050.dat"

# get energy shift:
eosfile = open(eosfilename,"r")
enrzero = np.float64(eosfile.readline().strip())*eps_gf
eosfile.close()

# get EOS data
eosdata = np.loadtxt(eosfilename,skiprows=1)
nrhos=len(eosdata[:,0])

# Format of the tables:
# col 0: log10(rho) [cgs]
# col 1: log10(press) [c=G=Msun=1]
# col 2: log10(eps) [c=G=Msun=1]
# col 3: ye
# col 4: temp [MeV]

tovinfo = tovinfoclass(nrhos)
tovinfo.polyK = 100.0
tovinfo.polyG = 2.0
tovinfo.nzones = 20000
tovinfo.rmax = 50.0
tovinfo.eostype = 3
tovinfo.minpress = 1.0e-18

tovinfo.logrhos[:] = np.log10(10.0e0**eosdata[:,0] * rho_gf)
tovinfo.eostable[:,0] = eosdata[:,1]
tovinfo.eostable[:,1] = eosdata[:,2]
tovinfo.eosepsshift = enrzero
tovinfo.eoslrhomin = tovinfo.logrhos[0]
tovinfo.eoslrhomax = tovinfo.logrhos[-1]
tovinfo.eosdlrho = tovinfo.logrhos[1]-tovinfo.logrhos[0]
tovinfo.eosdlrhoi = 1.0/tovinfo.eosdlrho

rho1 = 4.0e14
rho2 = 3.0e15
nsteps = 100
outdata = tov_sequence(rho1,rho2,nsteps,tovinfo)

mpl.plot(outdata[:,3]*inv_length_gf/1.0e5,outdata[:,1])
mpl.show()
